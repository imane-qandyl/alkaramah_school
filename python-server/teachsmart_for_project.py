...
class TeachSmartChatbot:
    ...
    def generate_learning_resource(self, student_age=8, aet_target="", context="", 
        ...
        """
        Generate learning resource using ONLY the trained model prediction
        """
        ...
        # Create assessment data from inputs
        assessment_data = self._create_sample_assessment(ability_level, context, aet_target)
        
        # Get ONLY the model prediction
        result = self.predict_student_progress(assessment_data)
        
        if result.get('success'):
            prediction = result['prediction']
            return {
                'success': True,
                'content': f"Predicted Condition: {prediction['condition']}\nConfidence: {prediction['confidence']:.0%}\nRecommendation: {prediction['advice']}",
                'timestamp': datetime.now().isoformat(),
                'metadata': {
                    'model_prediction_only': True,
...
- Acknowledge and celebrate incremental progress"""
    ...
    def format_resource(self, resource_data):
        """Format the resource for display - return only the content"""
        if not resource_data or not resource_data.get('success'):
            return "Error: Unable to format resource"
        
        # Return only the content without extra formatting
